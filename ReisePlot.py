...
axs.set_title("Optimierungen in chronolgischer Reihenfolge")
...
for i, v in enumerate(insert_times):
    axs.text(i, v, f"{v:.2f}s", ha='center', va='bottom')
for i, v in enumerate(lookup_times):
    axs.text(i + bar_width, v, f"{v:.2f}s", ha='center', va='bottom')

plt.tight_layout()
plt.savefig("reise1_output.png", format='png')
plt.show()
