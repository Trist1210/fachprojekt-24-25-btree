...
axs.bar([i + bar_width/2  for i in index], lookup_times, bar_width, label="Lookup Time")
axs.set_xlabel("")
axs.set_ylabel("Time (seconds)")
axs.set_title("Laufzeitverbesserung durch LookUp-Batching")
axs.set_yticks(np.arange(0, 1.1*max(max(insert_times), max(lookup_times)), 1))
